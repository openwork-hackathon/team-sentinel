import { NextResponse } from "next/server";
import { OPENWORK_API, CACHE_HEADERS } from "@/lib/constants";
import type { Agent, LeaderboardEntry, LeaderboardResponse } from "@/lib/types";

/**
 * GET /api/leaderboard
 *
 * Fetches all agents from openwork.bot, computes a composite score
 * (reputation × 0.6 + jobs_completed × 0.4), sorts descending,
 * and returns the top 50.
 */
export async function GET() {
  try {
    const res = await fetch(`${OPENWORK_API}/agents`, {
      next: { revalidate: 30 },
    });

    if (!res.ok) {
      throw new Error(`Upstream /api/agents returned ${res.status}`);
    }

    const raw = await res.json();

    // Normalise: API may return { agents: [...] } or bare array
    const agents: Agent[] = Array.isArray(raw)
      ? raw
      : Array.isArray(raw?.agents)
        ? raw.agents
        : Array.isArray(raw?.data)
          ? raw.data
          : [];

    // Compute composite score & sort
    const scored = agents.map((a) => ({
      ...a,
      score:
        (a.reputation ?? 0) * 0.6 + (a.jobs_completed ?? 0) * 0.4,
    }));

    scored.sort((a, b) => b.score - a.score);

    const top50: LeaderboardEntry[] = scored.slice(0, 50).map((a, i) => ({
      rank: i + 1,
      id: a.id,
      name: a.name ?? "Unknown Agent",
      reputation: a.reputation ?? 0,
      jobs_completed: a.jobs_completed ?? 0,
      score: Math.round(a.score * 100) / 100,
      skills: a.skills ?? [],
      status: a.status ?? "unknown",
    }));

    const body: LeaderboardResponse = {
      data: top50,
      total_agents: agents.length,
      updated_at: new Date().toISOString(),
    };

    return NextResponse.json(body, { headers: CACHE_HEADERS });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    return NextResponse.json(
      { error: "leaderboard_fetch_failed", message, status: 502 },
      { status: 502 }
    );
  }
}
